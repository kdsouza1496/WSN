...
// implementing network where nodes directly   
// transmit to base station and the LEACH protocol  
// where clustering of the nodes is done before   
// transmission.  
...
int NUM_NODES = DEFAULT_NUM;    // number of nodes in the network  
                // default is 50  
int NETWORK_X = DEFAULT_SIZE;   // X-size of network  
                // default is 100  
int NETWORK_Y = DEFAULT_SIZE;   // Y-size of network  
                // default is 100  
double B_POWER = DEFAULT_POW;   // initial battery power of sensors  
                // default is 0.75  
...
double CLUSTER_PERCENT = DEFAULT_PERCENT;  
...
double LEACH_AD_DISTANCE = LEACH_AD_DISTANCE_DEFAULT;  
  
                // the message length of the advertisement  
                // that there is a cluster head  
                // default is 16  
int LEACH_AD_MESSAGE = LEACH_AD_MESSAGE_DEFAULT;  
...
double SCHEDULE_DISTANCE = SCHEDULE_DISTANCE_DEFAULT;  
                ...
                // default is 16.  
int SCHEDULE_MESSAGE = SCHEDULE_MESSAGE_DEFAULT;  
...
//for(j = 0; j  TOTAL_ROUNDS; j++){  
    ...
    for(i = 0; i  NUM_NODES; i++){  
        if(network_LEACH[i].round  (j - recent_round) ||  
          (j - recent_round = 0)){  
